...
module.exports = function allOf(env, schemaStack, objectStack, options) {
	...
	if (Array.isArray(schema.items)) {
		for (i = 0, len = schema.items.length; i < len; i++) {
			errors = checkValidity(env, schemaStack.concat(schema.items[i]), objectStack.concat({ object: prop, key: i }), options);
			if (errors !== null) {
				return errors;
			}
		}
		...
		if (prop.length > len && schema.hasOwnProperty('additionalItems')) {
			if (typeof schema.additionalItems === 'boolean') {
				...
				for (i = len, len = prop.length; i < len; i++) {
					errors = checkValidity(env, schemaStack.concat(schema.additionalItems), objectStack.concat({ object: prop, key: i }), options);
					if (errors !== null) {
						return errors;
					}
				}
		...
		for (i = 0, len = prop.length; i < len; i++) {
			errors = checkValidity(env, schemaStack.concat(schema.items), objectStack.concat({ object: prop, key: i }), options);
			if (errors !== null) {
				return errors;
			}
		}
