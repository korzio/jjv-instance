...
module.exports = function validate(env, schemaStack, objectStack, options) {
    ...
    while (schemaStack.length && objectStack.length) {
        ...
        if (!(!schema.hasOwnProperty('$ref'))) {
            ...
            var schemaStackNew = resolve(env, schemaStack, schema.$ref);
            if (!schemaStackNew) {
                return { '$ref': schema.$ref };
            }

            schemaStack = schemaStack.concat(schemaStackNew);
            continue;
        }
        /** type **/
        ...
        if (!(!schema.hasOwnProperty('allOf'))) {
            schemaStack = schemaStack.concat(schema.allOf);
            continue;
        }
        /** oneOf **/
            ...
            for (var p in schema.dependencies) {
                if (schema.dependencies.hasOwnProperty(p) && prop.hasOwnProperty(p)) {
                    if (Array.isArray(schema.dependencies[p])) {
                        ...
                        schemaStack = schemaStack.concat(schema.dependencies[p]);
                        continue;
                    }
                }
            }
            ...
            if (options.removeAdditional && hasProp && schema.additionalProperties !== true && typeof schema.additionalProperties !== 'object') {
                ...
                if (schema.hasOwnProperty('additionalProperties')) {
                    if (typeof schema.additionalProperties === 'boolean') {
                        ...
                        for (i = 0, len = props.length; i < len; i++) {
                            schemaStack = schemaStack.concat(schema.additionalProperties);
                            objectStack = objectStack.concat([prop[props[i]]]);
                            continue;
                        }
                    }
                }
        ...
        if (!(!Array.isArray(prop) || !schema.hasOwnProperty('items'))) {
            ...
            if (Array.isArray(schema.items)) {
                for (i = 0, len = schema.items.length; i < len; i++) {
                    errors = validate(env, [schema.items[i]], [prop[i]], options);
                    if (errors) {
                        return errors;
                    }
                }
                ...
                if (prop.length > len && schema.hasOwnProperty('additionalItems')) {
                    if (typeof schema.additionalItems === 'boolean') {
                        ...
                        var objects = prop.slice(len);

                        schemaStack = schemaStack.concat(objects.map(function(){ return schema.additionalItems }));
                        objectStack = objectStack.concat(objects);
                        continue;
                    }
                }
            } else {
                schemaStack = schemaStack.concat(schema.items);
                objectStack = objectStack.concat(prop);
                continue;
            }
        }
        /** additionalItems **/
        ...
        if (!(!schema.additionalItems || Array.isArray(prop) || typeof schema.additionalItems === 'boolean')) {
            schemaStack = schemaStack.concat(schema.additionalItems);
            objectStack = objectStack.concat(prop);
            continue;
        }
        /** property **/
